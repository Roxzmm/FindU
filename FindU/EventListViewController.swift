...
class EventListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "eventCell", for: indexPath) as! eventPrototypeCell
...
//        cell.detailTextLabel?.text = events[indexPath.row].eventDescription
        ...
        cell.eventDescriptionLabel.text = events[indexPath.row].eventDescription
        
        if let photoData = events[indexPath.row].poster {
            cell.eventDescriptionLabel.isHidden = true
            cell.posterImage.image = UIImage(data: photoData)
        }
        return cell
    }
